    ...
    internal sealed class FacebookEventParser
    {
        ...
        internal static FacebookEvent ParseEvent(XmlNode node)
        {
            ...
            if (node != null)
            {
                ...
                {
                    ev.StartDate = DateHelper.ConvertDoubleToDate(double.Parse(XmlHelper.GetNodeText(node, "start_time"), CultureInfo.InvariantCulture));
                }
                if (!String.IsNullOrEmpty(XmlHelper.GetNodeText(node, "end_time")))
                {
                    ev.EndDate = DateHelper.ConvertDoubleToDate(double.Parse(XmlHelper.GetNodeText(node, "end_time"), CultureInfo.InvariantCulture));
                }
